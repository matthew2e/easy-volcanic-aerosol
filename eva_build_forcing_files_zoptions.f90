...
PROGRAM eva_build_forcing_file 
  ...
  INTEGER ::          &
       ...
       var_ml_ID     , & ! pointer to lev variable in netCDF file
  ...
  REAL, ALLOCATABLE ::       &
       ...
       levels(:)           , & ! levels for output
       z(:,:)                  ! height grid for EVA calculations
  REAL ::                    &
       !z(46)               , & ! height grid for EVA calculations
       so4_in(3)               ! sulfate triple to use in time loop
  INTEGER :: z_option = 4  ! 1=standard 1km grid, 2=0.5 km grid, 3=read from grid file, 4=self defined vector
  character(8)  :: date
  character(10) :: time
  ...
  IF (z_option.EQ.3) THEN
    ! In development -- this reads a cloumn vector, but we want z to be 2D
    ! (z,lat) 
    iret = nf90_open(TRIM(grid_filename), NF90_NOWRITE, ncid)
    IF (iret /= NF90_NOERR) STOP 'Error in opening grid file'
    iret = nf90_inq_dimid(ncid, "p_mid", VarID)
    iret = nf90_inquire_dimension(ncid, VarID, len = nz)
    allocate(z(nz,nlat))
    iret = nf90_get_var(ncid, VarID, z(:,:)  , start=(/1,1/) ,count=(/nz,nlat/))
    IF (iret /= NF90_NOERR) STOP 'Error in reading gridfile z'
  ELSE IF (z_option.EQ.4) THEN
    nz=20
    allocate(z(nlat,nz))
    DO i=1,nlat ! some dimension juggling going on here to ensure backwards compatibility
  ...
  do y=1,nyear
     ...
     do i=1,12
       ...
       do j=1,nlat
         call eva_ext_reff(lat,z(j,:),lat(j),so4_in,nlat,nz,ext550_vec,reff_vec)
         call eva_aop_profile(lat,z(j,:),lat(j),so4_in,lambda,nwl,nlat,nz,ext_vec,ssa_vec,asy_vec)
    ...
    iret = iret + nf90_def_var(ncid, 'ext'    , NF90_FLOAT, (/mlID,latID,wlID,timeID/), var_ext_ID)
    iret = iret + nf90_def_var(ncid, 'ssa'    , NF90_FLOAT, (/mlID,latID,wlID,timeID/), var_ssa_ID)
    iret = iret + nf90_def_var(ncid, 'asy'    , NF90_FLOAT, (/mlID,latID,wlID,timeID/), var_asy_ID)
    iret = iret + nf90_def_var(ncid, 'reff'   , NF90_FLOAT, (/mlID,latID,timeID/), var_reff_ID)
    ...
    iret = iret + nf90_put_att(ncid, var_ml_ID    , "long_name", "level")
    iret = iret + nf90_put_att(ncid, var_ml_ID    , "units"    , "")
    ...
    IF (iret /= 20*NF90_NOERR) STOP 'Error in creating file variable attributes'
    !
    iret = NF90_NOERR  
